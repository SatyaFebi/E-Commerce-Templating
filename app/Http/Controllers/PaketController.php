...
class PaketController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($namaPaket){
            ...
            $data = $request->only([
                'nama_paket', 'jenis', 'kategori', 'jumlah_pax', 'harga_paket', 'deskripsi' //harus sama dengan nama field di database !
                //Name pada form harus sama dengan nama field di database !
            ]);
            if($request->file('foto1') !== null) $data ['foto1'] = 
            $request->file('foto1')->store('Foto Paket');
            if($request->file('foto2') !== null) $data ['foto2'] = 
            $request->file('foto2')->store('Foto Paket');
            if($request->file('foto3') !== null) $data ['foto3'] = 
            $request->file('foto3')->store('Foto Paket');
    ...
    public function update(Request $request, string $id)
    {
        ...
        ->where('id', '=', $id)
        ->value('nama_paket');
        // d('nama paket baru: ' . $nama_paket . ' Nama paket Lama: ' .$nama_paket_lama);
        ...
        if(trim($nama_paket) === trim($nama_paket_lama)) {
            ...
            $foto1_lama = DB::table('pakets')
            ->where('id', '=', $id)
            ->value('foto1');
            $foto2_lama = DB::table('pakets')
            ->where('id', '=', $id)
            ->value('foto2');
            $foto3_lama = DB::table('pakets')
            ->where('id', '=', $id)
            ->value('foto3');
    ...
    {
        $foto1 = DB::table('pakets')
        ->where('id', '=', $id)
        ->value('foto1');
        $foto2 = DB::table('pakets')
        ->where('id', '=', $id)
        ->value('foto2');
        $foto3 = DB::table('pakets')
        ->where('id', '=', $id)
        ->value('foto3');
        ...
        ->with('pesan', 'The package you selected has been successfully deleted');
    }
}
